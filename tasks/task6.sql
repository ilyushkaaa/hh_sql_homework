CREATE INDEX idx_date_of_response ON responses (time_of_response);
--индекс оптимизирует группировку по дате отклика и ускоряет доступ к датам отклика
CREATE INDEX idx_vacancy_id_date_of_publication ON responses (vacancy_id, time_of_response);
